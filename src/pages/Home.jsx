...
const Home = ({ taskState }) => {
  ...
  const handleSubmit = (event) => {
    ...
    setTaskList(prev => [ ...prev , { id, task: inputTask, red, green, blue} ])
    ...
    <>
      <div className='w-[80vw] sm:w-[70vw] md:w-[60vw] lg:w-[40vw] h-[80vh] bg-[#001524] relative rounded-md overflow-y-scroll'>
        ...
        <div className='w-[90%] relative top-[35%] left-[5%]'>
          {taskList?.map((eachTask, index) => <Task key={index} index={index} id={eachTask.id} task={eachTask.task} red={eachTask.red} green={eachTask.green} blue={eachTask.blue} setTaskList={setTaskList}/>)}
        </div>
      </div>
    </>
  )
